...
<select name="pagesize" id="pagesize" class="p1" style="width:60px;" onchange="this.form.method='get';this.form.submit();">
    <?php
...
if ( ! function_exists('admin_category_breadcrumbs'))
{
	function admin_category_breadcrumbs($catid,$segment='')
	{
		...
		FROM wl_categories WHERE category_id='$catid' AND status!='2' ";		 
		  ...
		  if ($num_rows > 0)
		  {
				  ...
				  foreach( $query->result_array() as $row )
				  {
						...
						if ( has_child( $row['parent_id'] ) )
						{
								...
								$condtion_product   =  "JOIN wl_product_category as b ON a.products_id=b.ref_product_id WHERE  b.category_id='".$row['category_id']."' AND a.status!='2'";				
