...
function CountrySelectedBox($varg=array())
{	
	$CI = CI();	
	$var="";
	...
	***********************************************************/		
	...
	$contry_res=$CI->db->query("SELECT * FROM wl_location WHERE loc_type='country' AND status='1'")->result_array();
	foreach($contry_res as $key=>$val)
	{		
		if(is_array($varg['current_selected_val']))
		{
...
{
  function get_mail_logo()
  {
	return '<img src="'.theme_url().'mail_logo.png" />';
  }
}