	...
	public function change_status()
	{
			...
			if(is_array($arr_ids)){			
				...
				{	
					...
					$nlettr_res = $this->db->query("SELECT subscriber_email from wl_newsletters WHERE subscriber_id in ($str_ids)")->result_array();
					if(is_array($nlettr_res) && !empty($nlettr_res))
					{
						foreach($nlettr_res as $val1)
						{
						  $query = $this->db->query("UPDATE wl_customers SET is_subscribed='Yes' WHERE user_name ='".$val1['subscriber_email']."'");
						}
					}
				...
				{		
					...
					if(is_array($nlettr_res) && !empty($nlettr_res))
					{
						foreach($nlettr_res as $val1)
						{
						  $query = $this->db->query("UPDATE wl_customers SET is_subscribed='No' WHERE user_name ='".$val1['subscriber_email']."'");
						}
					}
					$this->session->set_userdata(array('msg_type'=>'success'));
