...
class paintservice extends MX_Controller{
	...
	public function index(){
		...
		$data['countries'] = $this->paintservice_model->get_countries();
	...
	public function concatequery($data){
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST'){}else{redirect('/paintservice', '');}
		if(!$this->input->post()){
			redirect('/paintservice', '');
		}
	...
	public function paintservice_result(){
	...
	public function paintservice_availibility($data,$vid){
		
		return $this->paintservice_model->check_vehicle_avail($data,$vid);
		
	}
	
	
	public function book_paintservice(){
		...
		$this->load->view('paintservice_payment_view',$Message);
	...
	public function save_paintservice(){
	...
	public function download_paintservice_docs($v_id){
		$vendor_id = $v_id;
