/****** Helpers.min.js Ver: 1.9.1 Copyright 2012, WeblinkIndia.Net Date: 08-08-2014 *******/
...
/*Browser alert*/
function upgrade(){$("body").append('<div style="position:fixed; bottom:0; z-index:999; left:0; text-align:center;background:#fbf98e; padding:5px; width:100%; font:12px/16px arial; border-bottom:1px solid orange">Oops..!! It seems that you are using <b><i>'+navigator.sayswho+"</i></b>, which is an Old version. Please upgrade your browser for a better web performance.</div>").css("padding-bottom","25px")}navigator.sayswho=function(){var e=navigator.userAgent,t,n=e.match(/(opera|chrome|safari|firefox|msie|trident(?=\/))\/?\s*(\d+)/i)||[];if(/trident/i.test(n[1])){t=/\brv[ :]+(\d+)/g.exec(e)||[];return"IE "+(t[1]||"")}if(n[1]==="Chrome"){t=e.match(/\bOPR\/(\d+)/);if(t!=null)return"Opera "+t[1]}n=n[2]?[n[1],n[2]]:[navigator.appName,navigator.appVersion,"-?"];if((t=e.match(/version\/(\d+)/i))!=null)n.splice(1,1,t[1]);return n.join(" ")}();var dgA=navigator.sayswho.split(" ");if(dgA[0]=="Chrome"&&dgA[1]<=34){upgrade()}if(dgA[0]=="Opera"&&dgA[1]<=11){upgrade()}if(dgA[0]=="safari"&&dgA[1]<=5){upgrade()}if(dgA[0]=="MSIE"&&dgA[1]<=9){navigator.sayswho="Internet Explorer "+dgA[1];upgrade()}if(dgA[0]=="firefox"&&dgA[1]<=28){upgrade()}
