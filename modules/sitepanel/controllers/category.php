...
class Category extends Admin_Controller
{
	...
	public function add()
	{
		...
		if($this->form_validation->run()===TRUE)
		{
			...
			{
			  /* For Individual */
			  $redirect_url = "category/index/type/individual";
			  $page_url = $this->config->item('individual_url_prefix')."/".$this->cbk_friendly_url;
			  $meta_array  = array(
			  ...
			  /* For Corporate */
			  $redirect_url = "category/index/type/corporate";
			  $page_url = $this->config->item('corporate_url_prefix')."/".$this->cbk_friendly_url;
			  $meta_array  = array(
							  'entity_type'=>$redirect_url,
							  'entity_id'=>$insertId,
							  'page_url'=>$page_url,
			  ...
			  {
				/* For Vendors */
				$redirect_url = "category/vendors";
				$page_url = $this->cbk_friendly_url."/".$this->config->item('cat_vendor_url_suffix');
				$meta_array  = array(
								'entity_type'=>$redirect_url,
								'entity_id'=>$insertId,
								'page_url'=>$page_url,
	...
	public function edit()
	{
	   ...
	   $this->form_validation->set_rules('category_image','Image',"file_allowed_type[image]|file_size_max[$img_allow_size]|check_dimension[$img_allow_dim]");
		...
		{	
			...
			/* Update Individual Meta */
			$redirect_url = "category/index/type/individual";
			$page_url = $this->config->item('individual_url_prefix')."/".$this->cbk_friendly_url;

			update_meta_page_url($redirect_url,$categoryId,$page_url);

			/* Update Corporate Meta */
			$redirect_url = "category/index/type/corporate";
			$page_url = $this->config->item('corporate_url_prefix')."/".$this->cbk_friendly_url;
	...
	public function delete()
	{
	  ...
	  else
	  {
		  ...
		  $condtion_product   =  "JOIN wl_product_category as b ON a.products_id=b.ref_product_id WHERE  b.category_id='".$catId."' AND a.status!='2'";

		  $total_product     =  count_products($condtion_product);
		  ...
		  else
		  {
			...
			$where = array('category_id'=>$catId);
			$this->category_model->safe_delete('wl_product_category',$where,TRUE);

			/* Delete Category Meta */
			/* Individual */
			$friendly_url_val = $this->config->item('individual_url_prefix')."/".$rowdata['friendly_url'];
			$where = array('entity_id'=>$catId,'page_url'=>$friendly_url_val);
			$this->category_model->safe_delete('wl_meta_tags',$where,TRUE);

			/* Corporate */
			$friendly_url_val = $this->config->item('corporate_url_prefix')."/".$rowdata['friendly_url'];
			$where = array('entity_id'=>$catId,'page_url'=>$friendly_url_val);
			$this->category_model->safe_delete('wl_meta_tags',$where,TRUE);
			...
			{
			  /* Vendor */
			  $friendly_url_val = $rowdata['friendly_url']."/".$this->config->item('cat_vendor_url_suffix');
			  $where = array('entity_id'=>$catId,'page_url'=>$friendly_url_val);
			  $this->category_model->safe_delete('wl_meta_tags',$where,TRUE);
			}
