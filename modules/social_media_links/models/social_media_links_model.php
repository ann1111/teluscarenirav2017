...
class social_media_links_model extends MY_Model
{
	...
	public function getsocial_media_links($opts=array())
	{
		...
		if($keyword!='')
		{
			$opts['condition'].= " AND media_title like '%".$keyword."%'";
		}
		...
		$result = $this->findAll('wl_social_media_links as a',$fetch_config);
		return $result;
	}
	...
	public function get_link_by_id($id)
	{
		...
		if($id>0)
		{
			$condtion = "status !='2' AND media_id=$id";
			$fetch_config = array(
														'condition'=>$condtion,							 					 
														'debug'=>FALSE,
														'return_type'=>"array"							  
													 );
			$result = $this->find('wl_social_media_links',$fetch_config);
			return $result;
		}
	}
}
