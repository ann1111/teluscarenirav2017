...
class Quotes extends Private_Controller
{
	...
	{	
		...
		$base_url               = "members/quotes/manage_quotes";
		...
		if($ajx_req===TRUE)
		{	
		  $this->load->view('members/quotes/load_quotes',$data);
		}
		else
		{
		  $this->load->view('members/quotes/view_quotes',$data);
		}
	}
    ...
    public function quote_details()
    {
	  ...
	  $condtion_array = array(
							  'where'=>"a.quotation_id ='".$quoteId."' AND a.posted_by ='".$this->userId."' AND c.status ='1'",
							  'fields'=>"SQL_CALC_FOUND_ROWS a.*,b.prod_title,b.prod_type,b.prod_for,b.short_description,b.friendly_url,b.status,b.user_status,c.company_name,c.status as company_status,c.friendly_url as company_url",
							  'offset'=>0,
							  'limit'=>1,
							  'debug'=>FALSE
	  ...
	  if(is_array($res_array) && !empty($res_array))
	  {
		...
		$base_url               = "members/quotes/quote_details/".$res_array['quotation_id'];
		...
		{
		  $data['res'] = $reply_res;
		  $data['company_name'] = $res_array['company_name'];
		  $this->load->view('members/quotes/load_quote_reply',$data);
		}
		else
		{
		  $this->load->view('members/quotes/view_quote_details',$data);
		}
	  }
	  else
	  {
		redirect('members/quotes/manage_quotes',''); 
	  }
   ...
   public function confirm_quote()
   {
	  ...
	  $condtion_array = array(
							  'where'=>"a.quotation_id ='".$quoteId."' AND a.posted_by ='".$this->userId."'",
							  'fields'=>'a.quotation_id,a.comments',
							  'offset'=>0,
							  'limit'=>1,
							  'debug'=>FALSE
							);
	  ...
	  if(is_array($res_array) && !empty($res_array))
	  {
		...
		$vendor_res = $this->db->select('user_name,first_name,last_name')->get_where('wl_customers',array('customers_id'=>$res_array['vendor_id']))->row();

		if(is_object($vendor_res))
		{
		  $first_name  = $vendor_res->first_name;
		  $last_name   = $vendor_res->last_name;	
		  $username    = $vendor_res->user_name;

		  $poster_name = $this->mres['first_name'].' '.	$this->mres['last_name'];

		  $poster_name = trim($poster_name);
		  					
		  $content    =  get_content('wl_auto_respond_mails','19');		
		  $subject    =  $content->email_subject;						
		  $body       =  $content->email_content;	
							  
		  $verify_url = '<a href="'.base_url().'users/login?ref=vendors/quotes/quote_details/'.$res_array['quotation_id'].'">Click here to view </a>';				
		...
		redirect('members/quotes/quote_details/'.$res_array['quotation_id'],''); 
	  }
	  else
	  {
		$this->session->set_userdata(array('msg_type'=>'error'));
   ...
   public function decline_quote()
   {
	...
	public function reply_quotation($res)
	{
		if($this->input->post('post')!='')
		{
		  ...
		  if($this->form_validation->run()===TRUE)
		  {
			  ...
			  $dtl_link = 'members/quotes/quote_details/'.$res['quotation_id'];
		...
		if(is_array($res_array) && !empty($res_array))
		{
		  ...
		  if($this->form_validation->run()===TRUE)
		  {
			  ...
			  $posted_data = array(
									  ...
									  'receiver_id'=>$res_array['vendor_id'],
									  'subject'=>$this->input->post('subject'),
			  ...
			  redirect('members/quotes/'.$red_url.'/'.$res_array['quotation_id'],'');
		  }
		  $data['res'] = $res_array;
	...
	public function remove_feedback()
    {
	  ...
	  if(is_array($res_array) && !empty($res_array))
	  {
		...
		$this->quote_model->safe_update('wl_quotation_feedback', $fd_data,$where ,FALSE );
