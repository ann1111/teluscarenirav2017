...
$prod_type = set_value('prod_type',$res['prod_type']); 
$prod_for = set_value('prod_for',$res['prod_for']);
...
{
  $product_categories = $db_category_ids;
}
...
if(empty($_POST))
{
  $why_to_choose=$res['why_to_choose'];
  if(!is_null($why_to_choose))
  {
	$why_to_choose = unserialize($why_to_choose);
  }
  $why_to_choose = !is_array($why_to_choose) ? array() : $why_to_choose;

  $detail_description=$res['detail_description'];
  if(!is_null($detail_description))
  {
	$detail_description = unserialize($detail_description);
  }
...
<div class="wrapper pt20 pb20" style="min-height:300px;">
  ...
  Service</label>
	  ...
	  <div class="fl w32 short_form ml16">
		...
		<p class="mt6">
		  <input name="prod_title" type="text" class="p7" style="width:100%" value="<?php echo set_value('prod_title',$res['prod_title']);?>">
	  ...
	  <div class="fr w32 short_form ml10">
		...
		<p class="mt6 fls bg-white" style="width:100%">
		  ...
		  $product_img  = @$db_product_images[$j]['media'];
		  $product_path = "product/images/".$product_img;
		   
		  if( $product_img!='' && file_exists(UPLOAD_DIR."/".$product_path) )
		  { 
			$img_del_fld = 'product_image'.$i.'_delete'; 
		  ?>
			<p class="fr red fs12" style="display:inline;"><a href="<?php echo base_url().'uploaded_files/'.$product_path;?>"  class="dg2">View</a>
		  | <input type="checkbox" name="<?php echo $img_del_fld;?>" value="Y" />Delete </p>
	  ...
	  <div class="short_form mt6 mb15">
		<textarea name="short_description" cols="100" rows="3" class="p7" style="height:70px; width:100%"><?php echo set_value('short_description',$res['short_description']);?></textarea>
	  ...
	  <div class="cb pb5"></div>
	  <input name="sbt_btn" type="submit" value="Update" class="btn3 radius-3 trans_eff">
