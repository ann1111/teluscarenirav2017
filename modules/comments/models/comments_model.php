...
class Comments_model extends MY_Model
 {
	 ...
	 {		
		...
		$query = "SELECT $sql_keys b.*,a.customers_id as poster_ref_id,IF(ISNULL(a.customers_id),b.author,CONCAT_WS(' ',a.first_name,a.last_name)) as mem_name $exselect FROM wl_review as b LEFT JOIN wl_customers as a ON  b.customer_id=a.customers_id $exjoin WHERE (b.status!='2' AND (a.status='1' OR  ISNULL(a.customers_id)) )   $excond  ORDER BY $order_by ";
		$query.=$limit_clause;
   ...
   public function get_entity_comments($cfg=array())
	 {		
		...
		$query = "SELECT $sql_keys b.*,a.customers_id as poster_ref_id,a.company_name $exselect FROM wl_review as b LEFT JOIN wl_customers as a ON  b.entity_id=a.customers_id $exjoin WHERE (b.status!='2' AND (a.status='1' ) )   $excond  ORDER BY $order_by ";
		$query.=$limit_clause;
		$comment_query=$this->db->query($query);
		$result=$comment_query->result_array();
		return $result;
