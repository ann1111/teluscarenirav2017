...
class MY_Form_validation extends CI_Form_validation
{
	...
	*/
	...
	{
		$dimen=explode("^^",$dimen);
		$label=(isset($dimen[1]))?$dimen[1]:"File";
		$dimen=$dimen[0];
		//trace($dimen);
		
		if (function_exists('getimagesize') && $file_name!='')
		{
			$file_name_tmp = $file_name['tmp_name'];
			$check_params = explode('~',$dimen);			

			$dim = explode('x',$check_params[0],2);
			...
			if(array_key_exists('1',$check_params))
			{
			  switch($check_params[1])
			  {
				case 'exact':
				  if($d[0] != $dim[0] || $d[1] != $dim[1])
				  {
					  
				   $this->CI->form_validation->set_message('check_dimension', "%s dimension  must be $check_params[0] )");				
				   return FALSE;
			...
			elseif(($d[0] > $dim[0]) || ($d[1] > $dim[1]))
			{
				
			 $this->CI->form_validation->set_message('check_dimension', "%s dimension  is too big. (max allowed dimension is $dimen )");				
			 return FALSE;
		  ...
		  {
			$this->CI->form_validation->set_message('check_dimension', "%s shoud not be other than image format");	
			return FALSE;
		  }
		}
	}
	...
	*/
	...
	public function is_valid_date2($val,$fld)
	{
	  if($val!='')
	  {
		...
		if(count($date_arr)<3)
		{
		  $this->CI->form_validation->set_message('is_valid_date2', 'Invalid '.$fld);
		  return FALSE;
		}
		...
		elseif(strtotime(date("$date_arr[0]-$date_arr[1]-$date_arr[2]"))<strtotime(date("Y-m-d")))
		{
		  $this->CI->form_validation->set_message('is_valid_date2', $fld." must not be less than current date");
		  return FALSE;
		}
	  }
	  return TRUE;
