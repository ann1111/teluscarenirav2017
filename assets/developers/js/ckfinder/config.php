...
if(preg_match('/(localhost|weblink4you.com|192.168.1.249)/',$host_name))
{
	preg_match("~^".$host_name."/([^/]+)(.*?)$~",$host_name.$_SERVER['PHP_SELF'],$matches);
	
	if($host_name=='192.168.1.249'){
		$project_name=explode("/",$matches[2]);
		$baseUrl='http://'.$host_name."/".$matches[1]."/".$project_name[1];
	}else{
